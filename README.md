...
node --import ./scripts/sites-env.mjs ./node_modules/wrangler/bin/wrangler.js d1 execute DB --local --config dist/server/wrangler.json --persist-to .wrangler/state --file drizzle/0006_lively_supreme_intelligence.sql
node --import ./scripts/sites-env.mjs ./node_modules/wrangler/bin/wrangler.js d1 execute DB --local --config dist/server/wrangler.json --persist-to .wrangler/state --file drizzle/0007_black_magneto.sql
npm run dev
```
...
- Atlas logs every stage it sees a send move to in D1 (`atlas_flightdeck_transitions`, migration 0006), with the time Atlas saw it: its own send's answer or a close ('atlas'), a read-back ('poll'), or later a webhook ('webhook'). Sends from before the log get one 'backfill' row with no time ("before tracking"). All writers go through `applyObservedStage` (`lib/flightdeck/transitions.ts`): one conditional insert per observation, a repeat of the latest stage is not logged, and a created or declined send never steps back from a late answer (needs more info may reopen to submitted). The log goes with its send when the project is deleted.
- Onboarding measures (`ONB_METRICS_ENABLED`, off unless exactly `true`; D-037): Atlas records one row per moment of a draft in D1 (`atlas_onboarding_metrics`, migration 0007) holding only `{draft_hash, kind, at}`, where the hash is sha256 of the Atlas project id. The moments are `draft-opened` (an empty draft's onboarding opened by someone who may edit it), `draft-saved` (its details first saved), `asked` (first ask to send; recorded once the ask marker lands), `sent` and `correction` (first seen filed, first seen needing more information). `summariseOnboardingMetrics` (`lib/flightdeck/metrics.ts`) reads time to first saved draft, ask-to-send and the correction rate off them. No field value is ever stored, and a failed write never fails the request it measures. In local dev set it in `.dev.vars` (only `ATLAS_*` variables pass through `.env.local`).
...
- **The FlightDeck Admin card** (plan 2026-09-25 adm-60, D-039 item 1): the app launcher shows a reserved built-in **FlightDeck Admin** card that opens `ATLAS_FLIGHTDECK_ADMIN_URL` in a new tab. This is the browser-facing admin address (for example `https://os.example.com/console/admin`), separate from `ATLAS_FLIGHTDECK_URL`, which may be a loopback address only the Atlas server can reach. It must be `https`, or plain `http` on a loopback host (`localhost`, `127.0.0.1`, `[::1]`) for local development, and must not carry credentials; an unset or refused value shows no card to anyone. The card is shown to the Atlas Super Admin and to members whose Atlas role id is listed in the comma-separated `ATLAS_FLIGHTDECK_ADMIN_ROLES`. **This is a visibility heuristic, not a FlightDeck entitlement:** Atlas does not know who is a FlightDeck super_admin or workspace admin, and discovering OS entitlements is out of scope. FlightDeck checks access again when the admin arrives, so a card shown to the wrong person leads to a refusal, not to admin controls. The id `flightdeck-admin` is reserved: Apps & connections cannot create or edit an app with that id (400), and a stored row using it is ignored.
...
- **The project page's FlightDeck card** (plan 2026-09-25 J1) shows where the project's FlightDeck request stands: Prepare, Continue draft (n of N for you), Waiting for Super Admin / Waiting for FlightDeck review, Fix request, or Project created with **Open in FlightDeck** (the Super Admin's link carries the workspace and project; an editor gets none, because their status projection holds no link). Its button opens the same onboarding form, so only the Super Admin sends. Optional `ATLAS_REQUESTER_REQUESTS=true` shows the card to members who can edit the project; unset (the default), only the Super Admin sees it. The local project field "Onboarding stage" is now "Readiness stage".
