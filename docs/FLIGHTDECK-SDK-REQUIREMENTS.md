...
- **OS routes** (inbound API route class, `Authorization: Bearer <inbound credential>` only, no cookies or `X-Workspace-Id`, scope `read:context`):
  - `GET /api/inbound/v1/context/workspaces` → `{ integrationId, instanceId?, workspaces: [{ id, label, enabled, isDefault }], generatedAt }`. Only workspaces listed in the OS admin setting `inbound.api.readWorkspaces`; empty means none. `instanceId` is the OS's stable instance id (proposal §8b decision 10). The OS integration line sends it; Atlas's strict DTO keeps it optional so an OS without it cannot break the context read. Atlas treats the value as **opaque** (any non-empty string up to 128 characters) and only stores and compares it: it does not assume a format, because a wrong guess inside the strict DTO would fail the whole context response. If the OS publishes no id, onboarding holds the send with `instance_unknown` and the context read is unaffected.
...
## Built, not yet proven end to end: project onboarding as a proposal

Added 2026-09-22 with the owner's approval of the onboarding plan ("take you recomendations", all 13 recommended answers). The OS half (workstream W1, the `project-onboarding` kind and its read-back) is shipped on the OS integration line and enabled locally (decision 11, D-035 item 6), but **not yet exercised end to end against the live OS**: nothing below has been proven against a running OS. It was tested against a fixture recorded from the contract (`tests/fixtures/os-project-onboarding.json`).
...
- **Atlas** (`lib/flightdeck/onboarding.ts`, `lib/flightdeck/onboard-route.ts`, `app/api/flightdeck/onboard/`): Super Admin only; the destination comes from the Review & send tab and is re-checked against fresh OS lists; the operation row and its idempotency key are stored in D1 before the call (a retry resends it without re-checking the destination); an unconfirmed send, or a filed one the OS answers 404 for, can be closed by the Super Admin so the next send takes a fresh key and adopts any request the OS holds; no background job: status is read back only while the Atlas Super Admin has Atlas open (their open form at most once a minute per send; their To FlightDeck list and dashboard up to two due sends a minute, each at most every five minutes), sharing the credential's rate limit with the context reads, and every view shows when each send was last checked; "Linked" only after `read:context` lists the promoted project under the OS `instanceId`. See [PROJECT-BRIDGE-CONTRACT.md](PROJECT-BRIDGE-CONTRACT.md) for the field allowlist and what is never sent.
- **OS switches the owner approved (decision 11), locally:** `INBOUND_PROJECT_ONBOARDING_ENABLED=true` in the launching shell and `inbound.api.integrationKinds=atlas:project-onboarding` (super_admin-writable). Until both are set a send is refused and nothing is filed. An OS without the kind or the scope answers `400` (unknown kind) or `403` (no `submit:proposal` scope); Atlas closes a first attempt refused that way as "Not sent" so a later send gets a fresh key. A retry refused that way keeps its reservation and key, because the earlier attempt may already have been filed.
